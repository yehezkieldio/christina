...
export const printTrace = (message: string): void => {
...
export const printCommitMessage = (message: string): void => {
  const width = Math.max(
    Math.min(terminalWidth(), COMMIT_MESSAGE_MAX_WIDTH) - 6,
    40
  );
  for (const line of wrapText(message, width)) {
    console.log(
      line.length === 0 ? styles.muted("│") : `${styles.muted("│")} ${line}`
    );
  }
  console.log("");
};
