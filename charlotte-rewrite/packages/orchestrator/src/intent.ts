...
import { type ChunkSummary, type SubTheme, type ThemeItem, themeResponseSchema, type Warning } from "@charlotte/schemas";
...
export function detectContradictions(summaries: readonly ChunkSummary[]): Warning[] {
  ...
  for (const [action, counteraction] of CONTRADICTORY_PAIRS) {
    ...
    if (hasAction && hasCounteraction) {
      return [{ kind: "contradiction", action, counteraction }];
    }
  }
...
function combinedDescription(summaries: readonly ChunkSummary[]): string {
  const parts = summaries.map((s) => s.summary.trim()).filter((s) => s.length > 0);
...
export function fallbackThemesFromSummaries(summaries: readonly ChunkSummary[]): ThemeItem[] {
  const totalFiles = summaries.reduce((sum, s) => sum + s.files.length, 0);
  return [{ title: "Code changes", description: combinedDescription(summaries), fileCount: totalFiles, scope: null }];
}
 ...
 * extraction. */
export function fallbackSubThemesFromSummaries(batch: readonly ChunkSummary[]): SubTheme[] {
  const totalFiles = batch.reduce((sum, s) => sum + s.files.length, 0);
  return [{ title: "Code changes", description: combinedDescription(batch), fileCount: totalFiles, scope: null }];
}
...
export function aggregateSubThemes(subThemes: readonly SubTheme[]): ThemeItem[] {
  ...
  for (const [scope, themes] of groups) {
    ...
    merged.push({ title, description, fileCount: totalFiles, scope });
  }
...
async function extractSubThemes(batch: readonly ChunkSummary[], options: IntentOptions): Promise<{ themes: SubTheme[]; promptTokens: number; completionTokens: number }> {
  options.signal?.throwIfAborted();
  const prompt = `${buildSystemPrompt()}\n\n${buildIntentPrompt(batch)}`;
  const result = await generateStructured({ model: options.model, schema: themeResponseSchema, prompt, signal: options.signal });

  const themes: SubTheme[] = result.object.themes
    .filter((t) => t.title.trim().length > 0 && t.description.trim().length > 0)
    .map((t) => ({ title: t.title.trim(), description: t.description.trim(), fileCount: t.fileCount, scope: t.scope }));

  if (themes.length === 0) {
    throw new Error("No valid sub-themes found in response");
  }

  return { themes, promptTokens: result.promptTokens, completionTokens: result.completionTokens };
}
...
async function extractIntentHierarchical(summaries: readonly ChunkSummary[], options: IntentOptions): Promise<IntentResult> {
  ...
  const batchResults = await mapWithConcurrency(batches, Math.max(1, Math.min(options.concurrencyLimit, batches.length)), async (batch) => {
    options.signal?.throwIfAborted();
    try {
  ...
  if (allSubThemes.length === 0) {
    return { themes: fallbackThemesFromSummaries(summaries), fallbackUsed: true, warnings: [], promptTokens, completionTokens };
  }

  const themes = aggregateSubThemes(allSubThemes);
  return { themes, fallbackUsed: anyFallback, warnings: [], promptTokens, completionTokens };
}
...
export async function extractIntent(summaries: readonly ChunkSummary[], options: IntentOptions): Promise<IntentResult> {
  options.signal?.throwIfAborted();
  ...
  try {
    ...
    return { themes, fallbackUsed: false, warnings, promptTokens: result.promptTokens, completionTokens: result.completionTokens };
  } catch {
    return { themes: fallbackThemesFromSummaries(summaries), fallbackUsed: true, warnings, promptTokens: 0, completionTokens: 0 };
  }
}
