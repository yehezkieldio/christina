...
function appendContext(prompt: string, context: PromptContext): string {
  let result = prompt;
...
export function buildDirectPrompt(diff: string, context: PromptContext = {}): string {
  const prompt = `Write a single-line Conventional Commit message for this diff:\n\n${diff}`;
  return appendContext(prompt, context);
}

export function buildChunkSummaryPrompt(diffChunkContent: string): string {
  return `Summarize the following diff chunk in one sentence, focused on what changed and why:\n\n${diffChunkContent}`;
}

export function formatSummariesForPrompt(summaries: readonly ChunkSummary[]): string[] {
  return summaries.map((summary) => `[${summary.files.length} files: ${summary.files.join(", ")}] ${summary.summary}`);
}

export function buildIntentPrompt(summaries: readonly ChunkSummary[]): string {
...
function describeTheme(theme: Pick<ThemeItem, "title" | "description" | "fileCount" | "scope">): string {
  const scoped = theme.scope ? ` (${theme.scope})` : "";
  return `- ${theme.title}${scoped}: ${theme.description} [${theme.fileCount} files]`;
}

export function buildReducePrompt(themes: readonly ThemeItem[], context: PromptContext = {}): string {
